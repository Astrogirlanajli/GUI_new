...
top_frame_r = tk.Frame(right_frame, bg="#0082b2", height=246.5, width=1060)
...
btn2.pack(side="right", padx=(15, 0))
...
spectrometer_menu_btn.pack(side="left")
...
interface_btn.pack(side="right", padx=(10, 0))
...
#Get value from wavelength
def get_wav_value():
        value_w = wavelength_entry.get()
        print(f"The entry value is: {value_w}")
...
#Get value from integration time entry
def get_integ_value():
        value_i = int_time_entry.get()
        print(f"The entry value is: {value_i}")
        print(value_i +value_w)
...
# Play Button
play_button = tk.Button(control_button_row, text="▶", **button_config, command = get_integ_value )
...
# Detect Button Section in bottom_frame!!!(Almost there )
